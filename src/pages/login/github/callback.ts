...
export async function GET(context: APIContext): Promise<Response> {
  ...
  try {
    ...
    const { rows } = await db.execute({
      sql: "SELECT * FROM OAUTH_ACCOUNT WHERE (PROVIDER_ID = 'github') AND (PROVIDER_USER_ID = ?);",
      args: [githubUser.id]
    })
    ...
    await db.batch([
      {
        sql: "INSERT INTO USER(ID, USERNAME, IMAGE) VALUES(?, ?, ?);",
        args: [userId, githubUser.login, githubUser.avatar_url]
      },
      {
        sql: "INSERT INTO OAUTH_ACCOUNT(PROVIDER_ID, PROVIDER_USER_ID, USER_ID) VALUES(?, ?, ?);",
        args: ["github", githubUser.id, userId]
      }
    ], "write")
